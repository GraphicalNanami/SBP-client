...
- `useEvents.ts`: Hook for fetching live events, mapping API data, and filtering.
- `liveEventsApi.ts`: API calls for live events (list, detail, register, unregister, my registrations).

## Data Flow
1. `useEvents` fetches live events from `GET /live-events`.
...
5. `EventDetail` checks registration state via `GET /live-events/me/registrations` when authenticated.
6. Register/Unregister uses `POST /live-events/{uuid}/register` and `DELETE /live-events/{uuid}/register`.
...
- 2026-02-07: **LIVE EVENTS INTEGRATION**:
  - Replaced mock events with `GET /live-events` data via `liveEventsApi`.
  - Added event detail fetch (`GET /live-events/{uuid}`) with loading/error states.
  - Implemented registration gating and state updates (register/unregister + registered count).
  - Updated calendar and popular cities to use live event data.
- 2026-02-07: **EVENT CREATION FEATURE**:
  - Created `CreateLiveEventPayload` type for event creation API.
  - Added `create()` method to `liveEventsApi` for `POST /live-events`.
  - Built comprehensive Create Event page at `/src/events/create` with:
    - Auth guard (redirects to sign-in if not authenticated)
    - Form with all required fields (title, dates, type, country, location, hosts)
    - Optional fields (description, banner URL, external URL, tags)
    - Dynamic host management (add/remove multiple hosts with names, roles, avatars)
    - Dynamic tag management (add/remove tags)
    - Responsive design with mobile-first approach
    - Form validation and error handling
    - Success redirect to created event detail page
  - Added "Create Event" button in events page hero (visible only to authenticated users)
  - Follows strict UI/Service separation pattern (no business logic in UI component)

## Future Enhancements
- Server-side filtering (pass filter state as query params to API)
